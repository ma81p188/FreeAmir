...
<script>
    function searchSelect({
        ...
        return {
            ...
            fetchSelectedLabel() {
                this.isLoading = true;
                const { id } = this.parseSelectedValue();
                fetch(`${this.url}?q=${encodeURIComponent(id || this.selectedValue)}`)
                    .then(r => r.json())
                    .then(data => {
            ...
            searchLocal(query) {
                ...
                this.initialOptions.forEach(block => {
                    ...
                    for (const gid in block.options) {
                        ...
                        const originalGroupName = items[0].groupName ?? '';
                        const normalizedGroupName = this.normalizeSearchText(originalGroupName);

                        const isGroupMatch = normalizedGroupName.includes(normalizedQuery);

                        const itemMatches = isGroupMatch ?
                            items :
                            items.filter(item =>
                                this.normalizeSearchText(item.text).includes(normalizedQuery)
                            );
                        ...
                        resultGroups[gid] = itemMatches.map(item => ({
                            ...item,

                            groupNameHighlighted: isGroupMatch ?
                                this.highlight(item.groupName, normalizedQuery) : null,

                            highlighted: !isGroupMatch ?
                                this.highlight(item.text, normalizedQuery) : null
                        }));
                    }
                    ...
                    .then(data => {
                        ...
                        data.forEach(block => {
                            ...
                            for (let gid in block.options) {
                                ...
                                const groupName = this.normalizeSearchText(items[0].groupName);
                                const isGroupMatch = groupName.includes(normalizedQuery);

                                let itemMatches = [];
                                if (!isGroupMatch) {
                                    itemMatches = items.filter(i =>
                                        this.normalizeSearchText(i.text).includes(normalizedQuery)
                                    );
                                }
                                ...
                                newGroups[gid] = (isGroupMatch ? items : itemMatches).map(i => ({
                                    ...i,
                                    groupNameHighlighted: isGroupMatch ?
                                        this.highlight(i.groupName, normalizedQuery) : null,
                                    highlighted: !isGroupMatch ?
                                        this.highlight(i.text, normalizedQuery) : null
                                }));
                            }
            ...
            highlight(text, query) {
                ...
                return String(text).replace(regex, '<span class="bg-yellow-200 dark:bg-blue-500">$1</span>');
            },
